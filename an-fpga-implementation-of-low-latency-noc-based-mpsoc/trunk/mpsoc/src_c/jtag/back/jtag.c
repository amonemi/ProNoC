...
//update chain code value
  ...
  // char * ptr;
   ...
   char* pronoc_work = getenv("PRONOC_WORK");
   fprintf(to_xsct, "source %s/toolchain/tcl/prog_mem_xsct.tcl\n",pronoc_work);

   printf("source %s/toolchain/tcl/prog_mem_xsct.tcl\n",pronoc_work);	
   ...
   fprintf(to_xsct_file,"source %s/toolchain/tcl/prog_mem_xsct.tcl\n",pronoc_work);	
...
int send_file_bin (char * file_name, unsigned int bits, unsigned int line ){
	char * ptr;
        printf("puts \"RESULT:[filehexdump %s \"%x\" %u %u]\n\"", file_name,chain_code,bits,line); 
	fprintf(to_xsct, "puts \"RESULT:[filehexdump %s \"%x\" %u %u]\"\n", file_name,chain_code,bits,line); 
	ptr=read_xsct();
#ifdef PRINT_TO_XSCT
	fprintf(to_xsct_file, "puts \"RESULT:[filehexdump %s \"%x\" %u %u]\"\n result: %s\n", file_name,chain_code,bits,line,ptr); 
#endif
        return 0;
