...
#define DEFAULT_INIT_WEIGHT     1
#define DEFAULT_MIN_PCK_SIZE    8 // must be larger than 1
...
int extract_traffic_data ( char * str,  task_t*  st) {
    ...
    st->dst=dst;
    st->bytes=bytes;
    st->initial_weight=(n>3 && initial_weight >0 )? initial_weight :DEFAULT_INIT_WEIGHT;
    st->min_pck_size=  (n>4 && min_pck_size>1 )? min_pck_size : DEFAULT_MIN_PCK_SIZE;
